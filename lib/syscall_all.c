...
void sys_ipc_recv(int sysno, u_int dstva)
{
	...
	if (cnt[ENVX(curenv->env_id)] == 0) {
		curenv->env_ipc_recving = 1;
	    curenv->env_ipc_dstva = dstva;
	    curenv->env_status = ENV_NOT_RUNNABLE;
	    sys_yield();
	} else {
		cnt[ENVX(curenv->env_id)]--;
		u_int id1 = q[ENVX(curenv->env_id)][cnt[ENVX(curenv->env_id)]][0];
		u_int id2 = q[ENVX(curenv->env_id)][cnt[ENVX(curenv->env_id)]][1];
		u_int value = q[ENVX(curenv->env_id)][cnt[ENVX(curenv->env_id)]][2];
		u_int srcva = q[ENVX(curenv->env_id)][cnt[ENVX(curenv->env_id)]][3];
		u_int perm = q[ENVX(curenv->env_id)][cnt[ENVX(curenv->env_id)]][4];
		envs[ENVX(id2)].env_ipc_dstva = dstva;
		envs[ENVX(id2)].env_ipc_value = value;
        envs[ENVX(id2)].env_ipc_from = id1;
        envs[ENVX(id2)].env_ipc_perm = perm;
		if (srcva != 0) {
	        p = page_lookup(envs[ENVX(id1)].env_pgdir, srcva, NULL);
	        if (p == NULL) {
	            return -E_INVAL;
         	}
	        page_insert(envs[ENVX(id2)].env_pgdir, p, dstva, perm);
	     }
		envs[ENVX(id1)].env_status = ENV_RUNNABLE;
	}
}
					 ...
					 u_int perm)
{
	...
	if (e->env_ipc_recving == 1) {
		...
		q[ENVX(e->env_id)][cnt[ENVX(e->env_id)]][0] = curenv->env_id;
		q[ENVX(e->env_id)][cnt[ENVX(e->env_id)]][1] = e->env_id;
		q[ENVX(e->env_id)][cnt[ENVX(e->env_id)]][2] = value;
		q[ENVX(e->env_id)][cnt[ENVX(e->env_id)]][3] = srcva;
		q[ENVX(e->env_id)][cnt[ENVX(e->env_id)]][4] = perm;
		cnt[ENVX(e->env_id)]++;
		curenv->env_status = ENV_NOT_RUNNABLE;
		sys_yield();
	}
