...
struct Page_list fast_page_free_list;	/* Free list of physical pages */
...
void page_init(void)
{
	...
	for (now = &pages[PPN(PADDR(freemem))]; page2ppn(now) < 12288; now++) {
		now->pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, now, pp_link);
	}

	for (; page2ppn(now) < 16384; now++) {
		now->pp_ref = 0;
...
struct Page* page_migrate(Pde *pgdir, struct Page *pp) {
	...
	char *p,*q;
	int i,j;
	for (p = page2kva(pp), q = page2kva(tp), i = 0; i<4096; p++, q++, i++) {
		*q = *p;
	}
		...
		if ((*pgdir_entry)&PTE_V) {
			...
			for (j = 0; j < PTE2PT; j++) {
				...
				if ((*pgtab_entry)&PTE_V) {
					if (pa2page(PTE_ADDR(*pgtab_entry)) == pp) {
						(pp->pp_ref)--;
						(tp->pp_ref)++;
						(*pgtab_entry) = page2pa(tp) | ((*pgtab_entry)&0xfff);
					}
				}
			}
		}
