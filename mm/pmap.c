...
int buddy_alloc(u_int size, u_int *pa, u_char *pi) {
	...
	if(temp==0) return -1;
	int i,j,flag;
	for (i=0;i*temp<8192;i++) {
		flag = 1;
		for (j=0;j<temp;j++) {
			if (extra[i*temp+j][0]==1) {
				flag = 0;
			}
		}
		if(flag) break;
	...
	if (i*temp>=8192) return -1;

	for(j=0;j<temp;j++) extra[i*temp+j][0]=1;
	extra[i*temp][1]=temp;
	*pa=0x2000000+i*temp*0x1000;
	for ((*pi)=0;temp;(*pi)++) temp = temp >> 1;
	(*pi)-=1;
	return 0;
}

void buddy_free(u_int pa) {
	u_int temp = (pa-0x2000000)>>12;
	if(temp>=8192) return;
	int i;
	for(i=0;i<extra[temp][1]&&temp+i<8192;i++) extra[temp+i][0]=0;
	extra[temp][1]=0;
}
