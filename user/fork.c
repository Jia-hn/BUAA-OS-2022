...
int make_shared(void *va) {
	...
	if (!((((Pde *)(*vpd))[vn >> 10] & PTE_V) && (((Pte *)(*vpt))[vn] & PTE_V))) {
		r = syscall_mem_alloc(0, vn << 12, PTE_V | PTE_R);
		if (r < 0) {
			return -1;
		}
	}
	u_int temp = ((Pte *)(*vpt))[vn];
	if ((temp & PTE_R) == 0) {
		return -1;
	}
	syscall_mem_map(0, vn << 12, 0, vn << 12, PTE_V | PTE_R | PTE_LIBRARY);
	return PTE_ADDR(temp);
}
/* Overview:
